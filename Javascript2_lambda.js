...
console.log(processLength(['foo', 'bar'], (num) => `${num} is the length`))
/**
...
console.log(processLastItem(['foo', 'bar'], (str) => `${str} is the last item`))
/**
 ...
 * 
 * Examples of usage of this higher-order function:
 * [1] Invoking `processSum` passing `[10, 20, 30]` and `callback`,
...
function processSum(numberList,callback) {
   return callback(numberList.reduce((acc, number) => acc + number));
   
}
console.log(processSum([10, 20, 30], (num) => num + 1000));
/**
...
console.log(processProduct(420, 69, num => `${num} pounds of weed`))
/**
...
console.log(processContains('foo', ['foo', 'bar'], item => item ? 'nice!' : 'sad'));
/**
...
//   runnerFullName.push(`${runners[i].last_name}, ${runners[i].first_name}`)
// }
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  ...
  let runnersBySize = runners.filter(function(currentValue){
    return currentValue.shirt_size === tShirtSize
  })
  return runnersBySize
}
console.log(getRunnersByTShirtSize(runners, '3XL'));
/**
...
//   let donationsArray = runners.map(function(donations){
//   return donations.donation;
// })

  let totalDonations = runners.reduce(function(acc, currentValue){
