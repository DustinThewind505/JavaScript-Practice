...
function get3rdCar(inventory) {
  let the3rd = inventory.find((item, index) => {
...
function getCarInfoByIndex(inventory, index) {
  let selectedCar = inventory[index];
...
function sortCarInventory(inventory) {
    ...
    return sortedInventory.sort().join(', ');
}
...
function getOlderCars(inventory, num) {
    const carsUnderMaxYear = []
    for(let i = 0; i < inventory.length; i++){
      if(inventory[i].car_year <= num){
        carsUnderMaxYear.push(inventory[i])
      }
    }
    return carsUnderMaxYear;
}
//console.log(getOlderCars(inventory, 2002));
/**
...
return car;
 // return `Damn! Your odometer is at ${}`;
}
console.log(carMaker(420).drive(60));
/// ////// END OF CHALLENGE /////////
/// ////// END OF CHALLENGE /////////
/// ////// END OF CHALLENGE /////////
